...
export class California extends StateScrapper implements StateScrapperInterface {
  ...
  protected playwrightMutex = new Mutex;
  ...
  public statutes = async () => {
    ...
    await this.runCheerio(async ({ $, request, enqueueLinks }) => {
      ...
      if(request.label == 'STATUTE') {
        console.log('statute')
        let bill_analysis_url:any = undefined;
        ...
        await self.runPlaywright(async ({ page, request, $ }) => {
          ...
          const ids = await page.$$eval('a.portletNav', (els: (SVGElement | HTMLElement)[]) => {
            var ids = [];
            var id;
            for(const n in els) {
              id = `this-is-an-id-${n}`;
  ...
  public administrativeCodes = async () => {
    ...
    await this.runCheerio(async ({ $, request, enqueueLinks }) => {
      ...
      if(request.label == 'ARTICLE') {
        await sleep(600 + Math.round(Math.random() * 800));
        enqueueLinks({
          strategy: 'same-domain',
          selector: '.co_genericWhiteBox li a',
          userData: {
      ...
      if(request.label == 'ITEM') {
        await sleep(500 + Math.round(Math.random() * 800));
