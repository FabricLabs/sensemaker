...
exports.Colorado = void 0;
const LexisScrapper_1 = require("./lexis/LexisScrapper");
const utils_1 = require("../utils");
class Colorado extends LexisScrapper_1.LexisScrapper {
    constructor() {
        super(Colorado.name);
        this.statutes = async () => {
        ...
        this.rulesOfCourt = async () => {
            ...
            await this.runCheerio(async ({ $, request, enqueueLinks }) => {
                ...
                if (request.label == 'YEAR') {
                    ...
                    const pdfs = $('.center-content a[href$=".pdf"]').toArray().map(a => {
                        const $a = $(a);
                        return {
                ...
                if (request.label == 'TITLE') {
                    for (let a of $('table table tr:nth-child(2) table:nth-child(3) a[href]')) {
                        ...
                        await self.runPlaywright(async ({ page, request, $, injectJQuery }) => {
                            ...
                            const ids = await page.$$eval('table table table:nth-child(9) tr td:nth-child(1) a', (els) => {
