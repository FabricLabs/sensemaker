...
exports.NewYork = void 0;
const StateScrapper_1 = require("./StateScrapper");
const utils_1 = require("../utils");
class NewYork extends StateScrapper_1.StateScrapper {
    constructor() {
        super(NewYork.name);
        this.statutes = async () => {
            let self = this;
            await this.runCheerio(async ({ request, $, enqueueLinks }) => {
                ...
                if (request.label == 'SECTION') {
                    ...
                    path.push($('.nys-openleg-result-container .nys-openleg-head-container:first .nys-openleg-result-title-headline').text().trim() +
                        ' - ' +
            ...
            await this.runCheerio(async ({ request, $, enqueueLinks }) => {
                ...
                if (request.label == 'START') {
                    ...
                    for (let i in as) {
                        ...
                        enqueueLinks({
                            strategy: 'same-domain',
                            urls: [(0, utils_1.completeUrl)($(a).attr('href'), 'https://ww2.nycourts.gov')],
                            userData: {
                                label: map_type[i],
                                path: [$(a).text().trim()]
                            }
                ...
                if (request.label == 'PDF-1') {
                    for (let a of $('.content.layout-content a[href$=".pdf"]').toArray()) {
                        await self.downloadRulesOfCourt((0, utils_1.completeUrl)($(a).attr('href'), 'https://ww2.nycourts.gov'), [...request.userData.path, $(a).text().trim().replace(/[\/]+/g, '.') + '.pdf']);
                    }
                }
                ...
                if (request.label == 'PDF-2:2-TITLE') {
                    for (let a of $('.content.layout-content a[href$=".pdf"]').toArray()) {
                        await self.downloadRulesOfCourt((0, utils_1.completeUrl)($(a).attr('href'), 'https://ww2.nycourts.gov'), [...request.userData.path, $(a).text().trim() + '.pdf']);
                    }
                }
                if (request.label == 'PDF-2:2-YELLOW') {
                    for (let a of $('.content.layout-content a[href$=".pdf"]').toArray()) {
                        await self.downloadRulesOfCourt((0, utils_1.completeUrl)($(a).attr('href'), 'https://ww2.nycourts.gov'), [...request.userData.path, $(a).closest('tr').find('td:first').text().trim() + '.pdf']);
                    }
                }
