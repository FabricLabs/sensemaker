...
exports.up = function (knex) {
  return knex.schema.alterTable('embeddings', function (table) {
    ...
    table.integer('title_embedding_id').unsigned().references('id').inTable('embeddings').onDelete('SET NULL');
    table.integer('body_embedding_id').unsigned().references('id').inTable('embeddings').onDelete('SET NULL');
  });
};
