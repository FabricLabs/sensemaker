  ...
  render () {
    ...
    return (
      <div>
        ...
        <Segment loading={agents.loading} style={{ maxHeight: '100%', height: 'auto' }}>
          <Table>
            <Table.Header>
              <Table.Row>
            ...
            <Table.Body>
              {agents && agents.agents && agents.agents.map((agent) => (
                <Table.Row key={agent.id}>
                  ...
                  <Table.Cell>{toRelativeTime(agent.created)}</Table.Cell>
                  <Table.Cell>{toRelativeTime(agent.updated)}</Table.Cell>
                </Table.Row>
              ))}
            </Table.Body>
          </Table>
