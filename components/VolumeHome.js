...
class VolumeHome extends React.Component {
  constructor (settings = {}) {
    super(settings);
    this.state = {
      searchQuery: '', // Initialize search query state
      filteredVolumes: [], // Initialize filtered volumes state
      ...
      <Segment className="fade-in" fluid style={{ marginRight: '1em', maxHeight: '100%' }}>
        ...
        {searching ? (
          ...
          searchQuery ? // if searching, goes this way
            (filteredVolumes && filteredVolumes.volumes && filteredVolumes.volumes.length > 0 ? (
              filteredVolumes.volumes.map((instance) => (
                <List.Item as={Card} key={instance.id}>
                  <Card.Content>
                    <h3><Link to={"/volumes/" + instance.id}>{instance.short_name}</Link></h3>
                    <Label.Group basic>
            ...
            ) : this.props.volumes && this.props.volumes.volumes && this.props.volumes.volumes.length > 0 ? (
              this.props.volumes.volumes.map((instance) => (
                <List.Item as={Card} key={instance.id}>
                  <Card.Content>
                    <h3><Link to={"/volumes/" + instance.id}> {instance.short_name} </Link> </h3>
                    <Label.Group basic>
