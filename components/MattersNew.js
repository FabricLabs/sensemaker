...
const { Link, Navigate, useNavigate } = require('react-router-dom');
    ...
    if (this.state.creating) {
      if (prevProps.matters !== matters && !matters.loading) {
        if (matters.creationSuccess) {
          ...
          this.props.navigateToMatter(matters.idCreated);
        } else {
...
function Matter(props) {
  ...
  const navigateToMatter = (id) => navigate(`/matters/${id}`);
  return <MattersNew navigateToMatter={navigateToMatter} {...props} />;
}
