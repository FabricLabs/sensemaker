...
class LibraryList extends React.Component {
  ...
  render () {
    ...
    const USER_IS_ALPHA = this.props.auth && this.props.auth.isAlpha || this.props.auth && this.props.auth.isAdmin || false;
    const USER_IS_BETA = this.props.auth && this.props.auth.isBeta || this.props.auth && this.props.auth.isAlpha || this.props.auth && this.props.auth.isAdmin || false;
        ...
        <section onClick={this.props.closeSidebars}>
          ...
          {USER_IS_ALPHA && ENABLE_CONTRACTS && (
            <Menu.Item as={Link} to='/contracts'>
              <div><Icon name='pencil' /> {!this.state.sidebarCollapsed && 'Contracts'} <div style={{ float: 'right' }}><Label size='mini'><code>alpha</code></Label> <Label size='mini' color='green'>New!</Label></div></div>
            </Menu.Item>
          )}
