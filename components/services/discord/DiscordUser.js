      ...
      <div style={{ minHeight: '100%', maxHeight: '100%', overflowY: 'auto' }}>
        <div className='uppercase'>
          ...
          <Breadcrumb style={{ marginLeft: '1em' }}>
            ...
            <Breadcrumb.Divider />
            <Breadcrumb.Section>{discord.user.name}</Breadcrumb.Section>
          </Breadcrumb>
        </div>
        <Segment className='fade-in' loading={discord?.loading} style={{ maxHeight: '100%' }}>
          <Header as='h1' style={{ marginTop: 0 }}>{discord.user.name}</Header>
        </Segment>
      </div>
    );
  }
...
function User (props) {
  const { id } = useParams();
  return <DiscordUser id={id} {...props} />;
}
