...
class DiscordMemberList extends React.Component {
  constructor (props) {
    super(props);
      ...
      <div style={{ minHeight: '100%', maxHeight: '100%', overflowY: 'auto' }}>
        <div className='uppercase'>
          ...
          <Breadcrumb style={{ marginLeft: '1em' }}>
            ...
            <Breadcrumb.Divider />
            <Breadcrumb.Section><Link to={`/services/discord/guilds/${discord.guild.id}`}>{discord.guild.name}</Link></Breadcrumb.Section>
            <Breadcrumb.Divider />
            <Breadcrumb.Section><Link to={`/services/discord/guilds/${discord.guild.id}/members`}>Members</Link></Breadcrumb.Section>
          </Breadcrumb>
        </div>
...
function MemberList (props) {
  const { id } = useParams();
  return <DiscordMemberList {...props} id={id} />;
}
