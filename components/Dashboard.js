      ...
      <jeeves-dashboard style={{ height: "100%" }} className="fade-in">
        ...
        >
          ...
          >
              ...
              <Header inverted>
                ...
                >
                  ...
                  <Popup.Content>
                    <p>
                      Send us an email:{" "}
                      <a href="mailto:support@jeeves.dev">support@jeeves.dev</a>
                    </p>
                    {/* <p><strong>Call Chuck!</strong> +1 (d00) p00-d00p</p> */}
                  </Popup.Content>
                </Popup>
                <img
                  src="/images/jeeves-tux.png"
                  class="icon"
                  style={{
                    height: "1.2em",
                    width: "1.2em",
                ...
                />{" "}
                J{this.state.sidebarCollapsed ? "" : "EEVES"}
                <Popup trigger={<Label>alpha</Label>}>
            ...
            <Menu.Item as={Link} to="/">
              <div>
                <Icon name="home" /> {!this.state.sidebarCollapsed && "Home"}
              </div>
            </Menu.Item>
            <Menu.Item as={Link} to="/cases">
              <div>
                <Icon name="briefcase" />{" "}
                {!this.state.sidebarCollapsed && "Cases"}
              </div>
            </Menu.Item>
            <Menu.Item as={Link} to="/conversations">
              <div>
                <Icon name="quote left" />{" "}
                {!this.state.sidebarCollapsed && "Conversations"}{" "}
                {this.state.conversationAlert ? (
            ...
            <Menu.Item as={Link} to="/settings">
              <div>
                <Icon name="cog" /> {!this.state.sidebarCollapsed && "Settings"}
              </div>
            </Menu.Item>
            {this.props.auth.isAdmin ? (
              <Menu.Item as={Link} to="/settings/admin">
                <div>
                  <Icon name="hammer" />{" "}
                  {!this.state.sidebarCollapsed && "Admin"}
                </div>
              </Menu.Item>
            ) : null}
            <Menu.Item
            ...
            >
              <div>
                <Icon name="sign-out" />{" "}
                {!this.state.sidebarCollapsed && "Logout"}
              </div>
            </Menu.Item>
            <Menu.Item style={{ borderBottom: 0 }}>
              ...
              <p style={{ marginTop: "2em" }}>
                <small class="subtle">
                  &copy; 2023 Legal Tools &amp; Technology, Inc.
                </small>
              </p>
              <p>
                <Label>
                  <strong>Status:</strong> {this.props.status || "disconnected"}
                </Label>
              </p>
            </Menu.Item>
          </Sidebar>
        ...
        <div
          id="main-content"
          style={{ marginLeft: "350px", paddingRight: "1em" }}
        >
          <Container fluid style={{ margin: "1em" }}>
            ...
            {this.state.debug ? (
              <div>
                ...
                <span>{this.props.isAdmin ? "yes" : "no"}</span>
                <br />
                <strong>
                  <code>isCompliant</code>:
                </strong>{" "}
                <span>{this.props.isCompliant ? "yes" : "no"}</span>
                <br />
                <strong>
                  <code>auth</code>:
                </strong>{" "}
                <code>
                  {this.props.auth
                    ? JSON.stringify(this.props.auth, null, "  ")
                    : "undefined"}
                </code>
            ...
            {this.state.isLoading ? null : (
              <Routes>
                ...
                <Route
                  ...
                  element={
                    <CaseView
                      fetchCase={this.props.fetchCase}
                ...
                <Route
                  ...
                  element={
                    <CaseHome
                      cases={this.props.cases}
                      fetchCases={this.props.fetchCases}
                    />
                  }
                />
                <Route
