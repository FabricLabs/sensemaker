  ...
  res.format({
    json: async () => {
      ...
      if (req.user?.state?.roles?.includes('admin')) {
        ...
        results = await this.db.select('fabric_id as id', 'id as dbid', 'fabric_id as slug', 'fabric_id', 'title', 'summary', 'created_at').from('conversations').where({ creator_id: req.user.id }).where('help_chat', 0).orderBy('created_at', 'desc');
