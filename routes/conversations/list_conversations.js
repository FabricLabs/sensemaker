  ...
  res.format({
    json: async () => {
      ...
      if (req.user?.state?.roles?.includes('admin')) {
        results = await this.db.select('c.id', 'c.fabric_id as slug', 'c.fabric_id', 'c.title', 'c.summary', 'c.created_at', 'username as creator_name','file_fabric_id').from('conversations as c').where('help_chat', 0).orderBy('created_at', 'desc').join('users', 'c.creator_id', '=', 'users.id');
      } else {
        results = await this.db.select('id', 'fabric_id as slug', 'c.fabric_id', 'title', 'summary', 'created_at', 'file_fabric_id').from('conversations').where({ creator_id: req.user.id }).where('help_chat', 0).orderBy('created_at', 'desc');
