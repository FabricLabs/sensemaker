...
module.exports = function (req, res, next) {
  ...
  res.format({
    json: async () => {
      const document = await this.db('documents').select('fabric_id as id', 'title', 'created_at', 'updated_at', 'mime_type', 'content', 'history', 'latest_blob_id').where('fabric_id', req.params.fabricID).orderBy('created_at', 'desc').first();
