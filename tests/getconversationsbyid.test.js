...
describe('Get Conversations By ID', function() {
  it('should retrieve conversation and messages', async function() {
    ...
    const fakeDb = {
      select: sinon.stub().resolves({ id: 1, title: 'Test Conversation', created_at: new Date(), log: [1, 2, 3], file_fabric_id: 'file_fabric_id' }),
    ...
    expect(fakeDb.select.calledOnceWith('id', 'title', 'created_at', 'log', 'file_fabric_id')).to.be.true;
  });
});
